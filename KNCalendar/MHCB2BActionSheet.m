...
@end
@implementation MHCB2BActionSheet
+(instancetype)styleDefault{
    MHCB2BActionSheet* sheet = [[MHCB2BActionSheet alloc]initWithFrame:CGRectMake(
                                                                                      0,
                                                                                      0,
...
- (UIDatePicker *)actionPicker{
    ...
    picker.frame = CGRectMake(0, 0,ScreenWidth ,150);
    NSDate *currentDate = [NSDate date];
    picker.date = currentDate; // 设置初始时间
...
//    picker.timeZone = [NSTimeZone timeZoneWithName:@"GTM+8"]; // 设置时区，中国在东八区
    picker.minimumDate = beforeDate; // 设置最小时间
    picker.maximumDate = afterDate; // 设置最大时间
    self.chosedDate = currentDate;
...
- (void)datePickerValueChanged:(UIDatePicker *) sender{
    ...
    self.chosedDate = select;
}

-(void)actionDone{
    if ([self.delegate respondsToSelector:@selector(actionDoneDateChoosed:)]) {
        [self.delegate actionDoneDateChoosed:self.chosedDate];
    }
}
