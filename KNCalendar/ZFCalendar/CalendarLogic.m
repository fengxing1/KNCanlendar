...
- (NSMutableArray *)reloadCalendarView:(NSDate *)date  selectDate:(NSDate *)selectdate needDays:(int)days_number dataModel:(KNDateChooseModel *)dateModel
{
    //如果为空就从当天的日期开始
    if(date == nil){
        date = [NSDate date];
    ...
    for (int i = 0; i <= months; i++) {
        ...
        //当前月份包含当月的天数
        [self calculateDaysInCurrentMonthWithDate:month andArray:calendarDays dataModel:dateModel];
        //当前月份包含下月份的天数
...
- (void)calculateDaysInCurrentMonthWithDate:(NSDate *)date andArray:(NSMutableArray *)array dataModel:(KNDateChooseModel *)dateModel
{
...
//        NSDateComponents *compents = []
        ...
        if (dateModel) {
            NSInteger m = dateModel.workDays,n = dateModel.restDays;
            
            NSDate *date = dateModel.firstWorkDay;
            int days = [NSDate getDayNumbertoDay:date beforDay:calendarDay.date];
            if (days < 0) {
                days = -days;
                int num = days % (m + n);
                if (num > 0 && num <= n) {
                    calendarDay.bworkDay = NO;
                }else{
                    calendarDay.bworkDay = YES;
                }
            }else{
                int num = days % (m + n);
                if (num >= 0 && num < m) {
